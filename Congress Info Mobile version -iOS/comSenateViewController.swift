...
class comSenateViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate {

    @IBOutlet weak var searchBar: UISearchBar!
    @IBOutlet weak var rightBarBtn: UIBarButtonItem!
    @IBOutlet weak var TblJSON_comSenate: UITableView!
    
    var arrRes = [[String:AnyObject]]()
    var arrRes_comSenate = [[String:AnyObject]]()
    var arrRes_filtered = [[String:AnyObject]]()
    var tableLoaded = false
    
    @IBAction func showSearchBar(_ sender: UIBarButtonItem) {
        if self.view.viewWithTag(6)?.isHidden == true {
            self.view.viewWithTag(6)?.isHidden = false
            self.rightBarBtn.image = #imageLiteral(resourceName: "Cancel-50")
        }
        else if self.view.viewWithTag(6)?.isHidden == false {
    ...
    override func viewDidLoad() {
        ...
        self.view.viewWithTag(6)?.isHidden = true //hide searchbar
                ...
                if let resData = swiftyJsonVar["results"].arrayObject {
                    ...
                    for  var tempRes in self.arrRes {
                        if(tempRes["chamber"] as? String == "senate") {
                            self.arrRes_comSenate.append(tempRes)
                        }
                    }
                    ...
                    self.arrRes_comSenate = self.arrRes_comSenate.sorted(by: positionSort)
                    self.arrRes_filtered = self.arrRes_comSenate
                }
                if self.arrRes.count > 0 {
                    self.tableLoaded = true
                    self.TblJSON_comSenate.reloadData()
                    SwiftSpinner.hide()
                }
            }
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "jsonCell_comSenate") as! CustomCell_Com
    ...
    func filterSearch(searchText: String)  {
        for  var tempRes in self.arrRes_comSenate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        //print(arrRes2Detail)
        let comDetailsView = segue.destination as! comDetailsViewController_senate
        let selectedRow = TblJSON_comSenate.indexPathForSelectedRow!.row
        if segue.identifier == "comSenateJsonCell" {
            comDetailsView.recData = [arrRes_filtered[selectedRow]]
        }
    }

    @IBAction func backToComSenateViewController(segue:UIStoryboardSegue) {
    }
    /*
    // MARK: - Navigation
