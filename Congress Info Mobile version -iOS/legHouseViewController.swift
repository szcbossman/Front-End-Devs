...
class legHouseViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate {

    @IBOutlet weak var legHouseTabBarItem: UITabBarItem!
    @IBOutlet weak var righBarBtn: UIBarButtonItem!
    @IBOutlet weak var searchBar: UISearchBar!
    @IBOutlet weak var TblJSON_legHouse: UITableView!
    
    var arrRes = [[String:AnyObject]]()
    var arrRes_legHouse = [[String:AnyObject]]()
    ...
    @IBAction func showSearchBar(_ sender: UIBarButtonItem) {
        if self.view.viewWithTag(1)?.isHidden == true {
            ...
            self.righBarBtn.image = #imageLiteral(resourceName: "Cancel-50")
        }
        else if self.view.viewWithTag(1)?.isHidden == false {
            self.view.viewWithTag(1)?.isHidden = true
            self.righBarBtn.image = #imageLiteral(resourceName: "Search-50")
        }
    ...
    override func viewDidLoad() {
        ...
        self.view.viewWithTag(1)?.isHidden = true //hide searchbar
        let attributes = [NSFontAttributeName:UIFont(name: "Arial", size: 20)]
        self.legHouseTabBarItem.setTitleTextAttributes(attributes, for: .normal)
            ...
            if((responseData.result.value) != nil) {
                ...
                if let resData = swiftyJsonVar["results"].arrayObject {
                    ...
                    for  var tempRes in self.arrRes {
                        if(tempRes["chamber"] as? String == "house") {
                            self.arrRes_legHouse.append(tempRes)
                        }
                    }
                    ...
                    self.arrRes_legHouse = self.arrRes_legHouse.sorted(by: positionSort)
                    self.arrRes_filtered = self.arrRes_legHouse
                }
                if self.arrRes.count > 0 {
                    self.tableLoaded = true
                    self.TblJSON_legHouse.reloadData()
                    SwiftSpinner.hide()
                }
            }
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell : UITableViewCell = tableView.dequeueReusableCell(withIdentifier: "jsonCell_legHouse")!
    ...
    func filterSearch(searchText: String)  {
        for  var tempRes in self.arrRes_legHouse {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        //print(arrRes2Detail)
        let legDetailsView = segue.destination as! legDetailsViewController_house
        let selectedRow = TblJSON_legHouse.indexPathForSelectedRow!.row
        if segue.identifier == "legHouseJsonCell" {
    ...
    */
    @IBAction func backToLegHouseViewController(segue:UIStoryboardSegue) {
    }
}
