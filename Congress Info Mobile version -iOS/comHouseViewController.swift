...
class comHouseViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate {

    
    @IBOutlet weak var searchBar: UISearchBar!
    @IBOutlet weak var rightBarBtn: UIBarButtonItem!
    @IBOutlet weak var TblJSON_comHouse: UITableView!
    
    var arrRes = [[String:AnyObject]]()
    var arrRes_comHouse = [[String:AnyObject]]()
    ...
    @IBAction func showSearchBar(_ sender: UIBarButtonItem) {
        ...
        else if self.view.viewWithTag(5)?.isHidden == false {
    ...
    override func viewDidLoad() {
        ...
        self.view.viewWithTag(5)?.isHidden = true //hide searchbar
                ...
                if let resData = swiftyJsonVar["results"].arrayObject {
                    ...
                    for  var tempRes in self.arrRes {
                        if(tempRes["chamber"] as? String == "house") {
                            self.arrRes_comHouse.append(tempRes)
                        }
                    }
                    ...
                    self.arrRes_comHouse = self.arrRes_comHouse.sorted(by: positionSort)
                    self.arrRes_filtered = self.arrRes_comHouse
                }
                if self.arrRes.count > 0 {
                    self.TblJSON_comHouse.reloadData()
                }
            }
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "jsonCell_comHouse") as! CustomCell_Com
    ...
    func filterSearch(searchText: String)  {
        for  var tempRes in self.arrRes_comHouse {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        //print(arrRes2Detail)
        let comDetailsView = segue.destination as! comDetailsViewController_house
        let selectedRow = TblJSON_comHouse.indexPathForSelectedRow!.row
        if segue.identifier == "comHouseJsonCell" {
            comDetailsView.recData = [arrRes_filtered[selectedRow]]
        }
    }

    @IBAction func backToComHouseViewController(segue:UIStoryboardSegue) {
    }
    /*
    // MARK: - Navigation
