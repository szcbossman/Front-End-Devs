...
class billNewViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate {
    
    @IBOutlet weak var searchBar: UISearchBar!
    @IBOutlet weak var rightBarBtn: UIBarButtonItem!
    @IBOutlet weak var TblJSON_bilNew: UITableView!
    ...
    override func viewDidLoad() {
        ...
        self.view.viewWithTag(4)?.isHidden = true //hide searchbar
        Alamofire.request("http://app102116-env.us-west-2.elasticbeanstalk.com?billsNew").responseJSON { (responseData) -> Void in
            if((responseData.result.value) != nil) {
                ...
                if self.arrRes.count > 0 {
                    ...
                    self.TblJSON_bilNew.reloadData()
                    SwiftSpinner.hide()
                }
            }
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "jsonCell_billNew") as! CustomCell_Bill
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        //print(arrRes2Detail)
        let bilDetailsView = segue.destination as! bilDetailsViewController_new
        let selectedRow = TblJSON_bilNew.indexPathForSelectedRow!.row
        if segue.identifier == "bilNewJsonCell" {
            bilDetailsView.recData = [arrRes_filtered[selectedRow]]
        }
    }
    

    @IBAction func backToBilNewViewController(segue:UIStoryboardSegue) {
    }
    /*
    // MARK: - Navigation
